...
git -C ansible-docker-moby merge v28.3.3
```
...
docker run -it \
  ...
  -e TARGET_ARCH=amd64 \
  -e PLATFORM="© 2018 BeagleCloud Team" \
  registry.cn-qingdao.aliyuncs.com/wod/docker-cli:v28.3.3-amd64 \
...
docker run -it \
  ...
  -e TARGET_ARCH=arm64 \
  -e PLATFORM="© 2018 BeagleCloud Team" \
  registry.cn-qingdao.aliyuncs.com/wod/docker-cli:v28.3.3-arm64 \
  bash -c "mkdir -p .tmp/$TARGET_ARCH && cp /usr/local/bin/docker .tmp/$TARGET_ARCH/ && cp /usr/libexec/docker/cli-plugins/docker-buildx .tmp/$TARGET_ARCH/"
```
